...
class Gif(commands.Cog):
    ...
    async def pout(self, i: discord.Interaction):
        ...
        async with session.get(
            "https://g.tenor.com/v1/search?q=%s&key=%s"
            % ("anime pout", self.__class__._tenor_api_key)
        ) as r:
            if r.status == 200:
        ...
        embed.set_image(
            url=(pouts["results"])[rand_index]["media"][0]["mediumgif"]["url"]
        )
    ...
    async def hug(
        ...
        async with session.get(
            "https://g.tenor.com/v1/search?q=%s&key=%s"
            % ("anime hug", self.__class__._tenor_api_key)
        ) as r:
            if r.status == 200:
                response = await r.read()
        ...
        if i.user.id == member.id:
            ...
            desc = "{} hugged {}, I suppose!".format(i.user.mention, member.mention)
        embed = discord.Embed(description=desc, color=discord.Colour.random())
        embed.set_image(
            url=(hugs["results"])[rand_index]["media"][0]["mediumgif"]["url"]
        )
        await i.response.send_message(embed=embed)
    ...
    async def smug(self, i: discord.Interaction):
        ...
        async with session.get(
            "https://g.tenor.com/v1/search?q=%s&key=%s"
            % ("anime smug", self.__class__._tenor_api_key)
        ) as r:
            if r.status == 200:
                response = await r.read()
        ...
        embed = discord.Embed(description=desc, color=discord.Colour.random())
        embed.set_image(
            url=(smugs["results"])[rand_index]["media"][0]["mediumgif"]["url"]
        )
        await i.response.send_message(embed=embed)
    ...
    async def pat(
        ...
        async with session.get(
            "https://g.tenor.com/v1/search?q=%s&key=%s"
            % ("anime pat", self.__class__._tenor_api_key)
        ) as r:
            if r.status == 200:
                response = await r.read()
        ...
        if i.user.id == member.id:
            ...
            desc = "{} patted {}, I suppose!".format(i.user.mention, member.mention)
        embed = discord.Embed(description=desc, color=discord.Colour.random())
        embed.set_image(
            url=(pats["results"])[rand_index]["media"][0]["mediumgif"]["url"]
        )
        await i.response.send_message(embed=embed)
