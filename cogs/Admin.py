...
class Admin(commands.Cog):
    ...
    @app_commands.describe(
        member="The user you want to kick, in fact!",
        reason="The reason for this kick, I suppose!",
    )
    async def kick(
        self, i: discord.Interaction, member: discord.Member, *, reason: Optional[str]
    ):
    ...
    @app_commands.describe(
        member="The user you want to ban, in fact!",
        reason="The reason for this ban, I suppose!",
    )
    async def ban(
        self, i: discord.Interaction, member: discord.Member, *, reason: Optional[str]
    ):
    ...
    async def clean_autocomplete(
        ...
        return [
            ...
            if current.lower() in direction.lower()
        ]
    ...
    @app_commands.describe(
        limit="Amount of messages you want to delete, in fact!",
        direction="In which direction you want to start deleting, I suppose?!",
    ...
    async def clean(
        ...
        if direction:
            direction = (
                self.dir_aliases[direction]
                if direction in self.dir_aliases
                else direction
            )
        ...
        await i.followup.send(
            content="Cleared by {}, I suppose!".format(i.user.mention)
        )

    @app_commands.command(name="purge")
    @app_commands.checks.has_permissions(administrator=True)
    @app_commands.describe(
        member="The member you want to delete the messages of, in fact!"
    )
    async def purge(self, i: discord.Interaction, member: Optional[discord.Member]):
        ...
        if member == self.bot.user:
            ...
            await i.response.send_message(
                "Which degenerate's messages do you want to yeet, I suppose?!"
            )
    ...
    async def toggle(self, ctx: commands.Context[Bot], cmdname: str):
        ...
        if ctx.command == cmd:
            ...
            await ctx.send(
                f"I have {status} the `{cmd.qualified_name}` command, in fact!"
            )
