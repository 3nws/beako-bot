...
class Admin(commands.Cog):
    ...
    async def clean(self, ctx, limit: int, direction: str = None, msg_id: int = None):
        ...
        direction = (
            self.dir_aliases[direction] if direction in self.dir_aliases else direction
        )
        ...
        if msg_id:
            ...
            if direction == "after":
                history = await ctx.channel.history(
                    limit=limit, after=msg, oldest_first=True
                ).flatten()
            elif direction == "before":
                history = await ctx.channel.history(
                    limit=limit, before=msg, oldest_first=False
                ).flatten()
            ...
            history = await ctx.channel.history(
                limit=limit, before=ctx.message, oldest_first=False
            ).flatten()
            for message in set(history):
                await message.delete()
        else:
