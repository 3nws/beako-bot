...
class Util(commands.Cog):
    ...
    async def avatar(self, ctx, member: discord.Member = None):
        ...
        if member:
            ...
            avatar_frame.add_field(
                name=str(ctx.author) + " requested",
                value=" their own avatar, I suppose!",
            )
    ...
    async def banner(self, ctx, member: discord.Member = None):
        ...
        if f"None.{file_extension}" in r:
            ...
            embed.add_field(
                name=str(ctx.author) + " requested",
                value=" their own banner, I suppose!",
            )
            embed.set_image(url=f"{r}")
        ...
        try:
            ...
            if pixiv_result != "" and twitter_result != "":
                sauce_frame.add_field(
                    name=f"{pixiv_result.title} by {pixiv_result.author_name} with {str(pixiv_result.similarity)} similarity",
                ...
                sauce_frame.add_field(
                    name=f"Image posted by {twitter_result.author_name} with {str(twitter_result.similarity)} similarity",
                    value=f"on [Twitter]({twitter_result.source_url})",
                )
                sauce_frame.set_thumbnail(url=twitter_result.thumbnail)

            sauce_frame.set_footer(
                text=f"on {str(ctx.author)}'s request, I suppose!",
