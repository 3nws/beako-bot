    ...
    async def get_item(
        ...
        if item_info is not None:
            ...
            async with session.get(self.base_url + "/items/" + url_name) as r:
                if r.status == 200:
                    response = await r.read()
                    item_detail = json.loads(response)["payload"]["item"][
                        "items_in_set"
                    ][0]
                else:
                    print("WarframeMarket down!")
            trading_tax: str = (
                item_detail["trading_tax"]
                if "trading_tax" in item_detail.keys()
                else "No trading tax value"
            )
            ducats: str = (
                item_detail["ducats"]
                if "ducats" in item_detail.keys()
                else "No ducats value"
            )
            mastery_level: str = (
                item_detail["mastery_level"]
                if "mastery_level" in item_detail.keys()
                else "No mastery level"
            )
            ...
            for i in range(len(drop_list)):
                desc += (
                    ...
                    else drop_list[i]["name"]
                )
            session = self.bot.session
            orders_sorted: List[Dict[Union[Dict[str, Any], str], Any]] = []
            async with session.get(
                self.base_url + "/items/" + url_name + "/orders"
            ) as r:
                if r.status == 200:
                    response = await r.read()
                    orders = json.loads(response)["payload"]["orders"]
                    online_orders = [
                        o for o in orders if o["user"]["status"] != "offline"
                    ]
                    filtered_types = [
                        o for o in online_orders if o["order_type"] == order_type
                    ]
                    orders_sorted = (
                        sorted(filtered_types, key=lambda ele: ele["platinum"])
                        if order_type != "buy"
                        else sorted(
                            filtered_types,
                            key=lambda ele: ele["platinum"],
                            reverse=True,
                        )
                    )
                else:
            ...
            await interaction.response.send_message(
                "I couldn't find that item, in fact!"
            )
